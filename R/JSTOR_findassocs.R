...
#' 
#' @description Generates a plot of the top n words in all the documents in ranges of years that positively correlate with a given word. For use with JSTOR's Data for Research datasets (http://dfr.jstor.org/).
...
#' @examples 
#' ## findassocs <- JSTOR_findassocs(corpus, n = 10, "pirates", topn = 100)
#' ## findassocs <- JSTOR_findassocs(corpus, n = 5, corlimit=0.6, plimit=0.001)


JSTOR_findassocs <- function(corpus, n=10, word, corlimit=0.4, plimit=0.05, topn=20, biggest=5){
...
years <- split(uniqueyears, ceiling(x/as.numeric(n)))
